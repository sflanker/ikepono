...
def coco_eval_summarize_mlflow(coco_eval, iou_typ, epoch):
        def _summarize(ap=1, iouThr=None, areaRng='all', maxDets=100):
            p = coco_eval.params
            iStr = ' {:<18} {} @[ IoU={:<9} | area={:>6s} | maxDets={:>3d} ] = {:0.3f}'
            titleStr = 'Average Precision' if ap == 1 else 'Average Recall'
            typeStr = '(AP)' if ap == 1 else '(AR)'
            iouStr = '{:0.2f}_to_{:0.2f}'.format(p.iouThrs[0], p.iouThrs[-1]) \
            ...
            mlflow.log_metric(f"{titleStr}_{iouStr}_{areaRng}_{maxDets}", mean_s, step=epoch)
            return mean_s
...
class CocoEvaluator:
    ...
    def summarize(self):
        for iou_type, coco_eval in self.coco_eval.items():
            ...
            coco_eval.summarize()
            coco_eval_summarize_mlflow(coco_eval, iou_type, self.epoch)
